<template>
  ...
  <div class="report-card" v-else v-bind="$attrs">
    ...
    <template v-else-if="!arrayIsEmpty">
      <div class="label">
        <p v-if="model.title">آزمون جامع {{ model.title }}</p>
          ...
          <div class="chart-container">
            ...
            <div class="user-info animate__animated animate__fadeIn">
              <div class="info-div">
                <p>تراز کاربر</p>
                <span>
                  {{ +results[0]?.taraz.toFixed(2) }}
                </span>
              </div>
              <div class="info-div">
                <p>رتبه کاربر</p>
                <span> {{ +results[0]?.rate.toFixed(2) }} </span>
              </div>
            </div>
          </div>
          <!-- Btns -->
...
export default defineComponent({
  ...
  setup(props) {
    ...
    (async () => {
      if (props.id) {
        const res = await StudentExamApi.getResult(route.params.id as any);
        ...
        if (model.value.exam) {
          results.value = model.value.exam.results;
        }
        ...
        unAnswered.value =
          model.value.totalQuestions - model.value.totalAnswers;
      }
    ...
    const goToHamnavard = () =>
      router.push({
        name: 'Hamnavard',
        params: { percent: model.value.percent || 0 }
      });
